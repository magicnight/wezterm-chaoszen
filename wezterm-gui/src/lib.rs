...
//!
//! Slice 1b.4.a (predecessor): added [lib] target with run_with_mux
//! API shell (stubbed body).
//! Slice 1b.4.b.1: migrated 24 child modules + helper fns from
//! main.rs into this lib so the LIB API (run_with_mux, called by
//! chaoszen-app in 1b.4.b.2) can reach into termwindow / frontend /
//! etc. main.rs is reduced to a ~30-line binary-edge shim.
//!
//! Public API surface:
//!   - `run_with_mux::*` — the lib API for chaoszen-app
//!   - `run_cli` — CLI dispatcher (called by main.rs shim only)
//!   - `notify_on_panic` — panic hook setup
//!   - `terminate_with_error` — error-path termination
//!   - `frontend::shutdown` — re-exported for the binary cleanup path
//!
//! All other modules and helpers are `pub(crate)`.

// Don't create a new standard console window when launched from the windows GUI.
#![cfg_attr(not(test), windows_subsystem = "windows")]

// === use declarations (moved from main.rs) ===
...
// === Module declarations (moved from main.rs, with pub(crate) prefix) ===
pub(crate) mod colorease;
pub(crate) mod commands;
pub(crate) mod customglyph;
pub(crate) mod download;
pub(crate) mod frontend;
pub(crate) mod glyphcache;
pub(crate) mod inputmap;
pub(crate) mod overlay;
pub(crate) mod quad;
pub(crate) mod renderstate;
pub(crate) mod resize_increment_calculator;
pub mod run_with_mux;
pub(crate) mod scripting;
pub(crate) mod scrollbar;
pub(crate) mod selection;
pub(crate) mod shapecache;
pub(crate) mod spawn;
pub(crate) mod stats;
pub(crate) mod tabbar;
pub(crate) mod termwindow;
pub(crate) mod unicode_names;
pub(crate) mod uniforms;
pub(crate) mod update;
pub(crate) mod utilsprites;

// === Public re-exports ===
pub use run_with_mux::{run_with_mux, RunWithMuxOpts, SetupTask};
pub use frontend::shutdown;

#[cfg(feature = "dhat-heap")]
#[global_allocator]
static ALLOC: dhat::Alloc = dhat::Alloc;

pub(crate) use selection::SelectionMode;
pub(crate) use termwindow::{set_window_class, set_window_position, TermWindow, ICON_DATA};
...
pub(crate) fn run_ssh(opts: SshCommand) -> anyhow::Result<()> {
...
pub(crate) fn run_serial(config: config::ConfigHandle, opts: SerialCommand) -> anyhow::Result<()> {
    if let Some(cls) = opts.class.as_ref() {
        crate::set_window_class(cls);
    }
    if let Some(pos) = opts.position.as_ref() {
...
pub(crate) fn build_initial_mux(
    config: &ConfigHandle,
    default_domain_name: Option<&str>,
    default_workspace_name: Option<&str>,
) -> anyhow::Result<Arc<Mux>> {
...
pub(crate) fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    if let Some(cls) = opts.class.as_ref() {
        crate::set_window_class(cls);
    }
    if let Some(pos) = opts.position.as_ref() {
...
pub fn terminate_with_error(err: anyhow::Error) -> ! {
...
pub(crate) fn run_show_keys(config: config::ConfigHandle, cmd: &ShowKeysCommand) -> anyhow::Result<()> {
